				...
				if((tab[i] == '<')&&(tab[i+6] == '\n')){
					switch (tab[i+1]){
						...
						case 'c':
								...
								couple = -1*(data -100000.05f) ; // retrait de l'offset ajouté par la RPI lors de l'envoi
								break;
						//vitesse lineaire
						case 'v' :
								memcpy(&data,&tab[i+2],sizeof(data));
								vit_lin = -1*(data -100000.05f) ;
								break;
						//battery
						case 'b' :
								memcpy(&data,&tab[i+2],sizeof(data));
						...
						case 's' :
								memcpy(&data,&tab[i+2],sizeof(data));
								vit_ang = -1*(data -100000.05f)*180.0f/3.142f; //conversion en degrés
								break;
						//angle
						case 'p' :
								memcpy(&data,&tab[i+2],sizeof(data));
								angle = -1*(data -100000.05f)*180.0f/3.142f; //conversion en degrés
								break;
						//etat communication
						case 'e' :
								memcpy(&data,&tab[i+2],sizeof(data));
...
gboolean update_info(GtkWidget *widget){
	...
	if (vit_lin > 6.0 && line == FALSE){
		str = strcpy(str,"Vitesse trop élevée !");
		g_utf8_make_valid(str,strlen(str));
		line = TRUE;
	}
	else if (vit_lin > 6.0 && line == TRUE){
		str = strcat(str,"\nVitesse trop élevée");
		g_utf8_make_valid(str,strlen(str));
	}
