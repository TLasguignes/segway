...
// variable pour detection de m�me consigne de raspberry pi 
float old_cons;
int cmpt_egal = 0;

//	variable de l'�tat arr�t urgence declecnh� par Raspberry Pi 
int arret = 1;
...
/**
  * @brief  Interruption d�clench� par Accelerometre � 94 HZ (Interruption Principale de syst�me)
  * @param  None
  * @retval None
  */
...
{ 
	 ...
	 * Si on ne re�oit pas de donn�es de Raspberry Pi plus de 500ms, le LED clignote et le consigne_new =0
	 */
	watchdog++;
	if (watchdog >= 50){
	...
	// Lecture de donn�e et remplir buffer envoyer, puis envoyer de donn�e 
	Trait_Gyro_Acc ();
...
/**
  * @brief  Interruption par DMA de ADC, le boucle de courant est fait ici �galement 
  * @param  None
  * @retval None
  */
void DMA1_Channel1_IRQHandler (void)
{
	...
	if (DMA_GetITStatus(DMA1_IT_TC1) != RESET) {	//test pas utile car seule
	    ...
	    STM_EVAL_LEDToggle(LED9);
			...
			if (ConfigOK==1){ 								//	une fois la configuration du syst�me est fait
				Trait_b_courant();							//	execution boucle de courant une fois ADC a trouv� la valeur moyenne de courant
			}
		}
...
{
  static int rx_index = 0;		// variable intern pour index de trame re�u
 	unsigned char tampon = 0; 	// variable temporaire pour tester le premier caractere de trame 
	float value;								// variable temporaire pour valeur de donn�e de trame
  ...
  { 
		...
		watchdog = 0;																					
		// Remis le watchdog � 0 car il y a encore communication de USART 
		
		/* Detection de trame complet de Raspberry Pi
				- On interpr�te le carat�re de trame pour indentifier le debut de trame
				- Si c'est bien le d�but, on le prend et le stocker dans un buffer
		*/
		if (tampon == '<'){
			rx_index = 1;
			RX_USART[0] = '<';
    ...
    if (rx_index > (sizeof(RX_USART) - 1)){
			...
			if (RX_USART[0] == '<'&& RX_USART[6] == '\n') 
			{
				...
				// interpretation de type de donn�e dans le trame par label de trame
				switch (RX_USART[1]){
					//cas de reception de trame de consigne 
					case 'c' : 	Consigne_OK=1;
											Consigne_new=value/0.80435f;
											// detection si le consigne ancien est le m�me 
